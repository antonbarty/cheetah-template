...
;;
pro process_module, steffen_base_store, steffen_offset_store, manuela_dark_file, manuela_pcdrs_file, $
			file_format, module_name, steffen_h5_field, g3=g3, outdir=outdir
	...
	print, '    steffen_base_store:   ', steffen_base_store
	print, '    steffen_offset_store: ', steffen_offset_store
	print, '    manuela_dark_file:    ', manuela_dark_file
	print, '    manuela_pcdrs_file:   ', manuela_pcdrs_file

	print, '    steffen_h5_field:     ', steffen_h5_field
	...
	;; Read Manuela's PCDRS calibration data
	print,'Reading: ', manuela_pcdrs_file, '    h5_block=', manuela_h5_field
	;m_pcdrs_offset = read_h5(manuela_pcdrs_file, field=manuela_h5_field+'/offset')
	;m_pcdrs_threshold = read_h5(manuela_pcdrs_file, field=manuela_h5_field+'/threshold')
	;m_pcdrs_gainlevel_mean = read_h5(manuela_pcdrs_file, field=manuela_h5_field+'/gainlevel_mean')
	;m_pcdrs_slope = read_h5(manuela_pcdrs_file, field=manuela_h5_field+'/slope')
	...
	;; Read Steffen's offset file
	print,'Reading: ', steffen_offset_store, '    h5_block=', steffen_h5_field
	s_noise_field = steffen_h5_field+'/Noise/0/data'
	s_offset_field = steffen_h5_field+'/Offset/0/data'
	s_thresh_field = steffen_h5_field+'/Threshold/0/data'
	o_noise = read_h5(steffen_offset_store, field=s_noise_field)
	o_offset = read_h5(steffen_offset_store, field=s_offset_field)
	o_thresh = read_h5(steffen_offset_store, field=s_thresh_field)

	;; Read Steffen's base file
	print,'Reading: ', steffen_base_store, '    h5_block=', steffen_h5_field
	s_baseoffset_field = steffen_h5_field+'/BaseOffset/0/data'
	s_gain_field = steffen_h5_field+'/RelativeGain/0/data'
	b_offset = read_h5(steffen_base_store, field=s_baseoffset_field)
	b_gain = read_h5(steffen_base_store, field=s_gain_field)
	b_thresh = read_h5(steffen_base_store, field=s_thresh_field)


	;; Transpose Steffen's constants (Cheetah uses Manuela's layout, which is the transpose of Steffen's layout)
	;o_noise = transpose(o_noise)
	;o_offset = transpose(o_offset)
	;o_thresh = transpose(o_thresh)
	;b_offset = transpose(b_offset)
	;b_gain = transpose(b_gain)
	;b_thresh = transpose(b_thresh)
	...
	;gain_out = process_steffen_gain(b_gain)
	;gain_out = process_pcdrs_gain(b_gain, m_pcdrs_slope)
	...
	;thresh_out = process_thresh(m_dark_thresh)
	...
	;badpix_out2 = process_badpix_gain(gain_out)
...
pro process_agipd_calib, steffen_base_store, steffen_offset_store, manuela_dark_file, manuela_pcdrs_file, $
	...
	if not file_test(steffen_base_store) then begin
		print,'Error: can not find file'
		print, steffen_base_store
		return
	endif

	if not file_test(steffen_offset_store) then begin
		print,'Error: can not find file'
		print, steffen_offset_store
		return
	endif
	...
	for i=0, n_elements(modules_to_process)-1 do begin
		...
		process_module, steffen_base_store, steffen_offset_store, manuela_dark_file, manuela_pcdrs_file, $
			file_format, modules_to_process[i], steffen_h5_field[i], g3=g3, outdir=outdir
